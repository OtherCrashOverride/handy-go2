...
#include "system.h"
      ...
      int C65C02::GetPC(void) { return mPC; }

      void C65C02::xILLEGAL(void)
      {
         //char addr[1024];
      ...
      void C65C02::CMemMapPoke(ULONG addr, UBYTE data)
      {
      ...
      UBYTE C65C02::CMemMapPeek(ULONG addr)
      {
         UBYTE retval=0;
...
void C65C02::CpuPoke(ULONG addr, UBYTE data)
{
    switch (addr >> 8)
    {
        ...
        case 0xFC:
            if(mSusieEnabled)
            {
               mSystem.mSusie->Poke(addr, data); // &Susie_LynxBase;
            }
            else
            {
               mSystem.mRam->Poke(addr, data); // &CRam_LynxBase;
            }
            break;
        ...
        case 0xFD:
            if(mMikieEnabled)
            {
               mSystem.mMikie->Poke(addr, data); // &CMikie_LynxBase;
            }
            else
            {
               mSystem.mRam->Poke(addr, data); // &CRam_LynxBase;
        ...
        case 0xFE:
            if(mRomEnabled)
            {
               mSystem.mRom->Poke(addr, data); // &mRom.LynxBase;
            }
            else
            {
               mSystem.mRam->Poke(addr, data); // &CRam_LynxBase;
        ...
        case 0xFF:
            ...
            {
                CMemMapPoke(addr, data); // &CMemMap_LynxBase;
            }
            else if(mVectorsEnabled)
            {
               mSystem.mRom->Poke(addr, data); // &mRom.LynxBase;
            }
            else
...
UBYTE C65C02::CpuPeek(ULONG addr)
{
    switch (addr >> 8)
    {
        // FC00-FCFF Susie area
        case 0xFC:
            if(mSusieEnabled)
            {
               return mSystem.mSusie->Peek(addr); // &Susie_LynxBase;
            }
            else
            {
               return mSystem.mRam->Peek(addr); // &CRam_LynxBase;
            }
            break;
        ...
        case 0xFD:
            if(mMikieEnabled)
            {
               return mSystem.mMikie->Peek(addr); // &CMikie_LynxBase;
            }
            else
            {
               return mSystem.mRam->Peek(addr); // &CRam_LynxBase;
        ...
        case 0xFE:
            if(mRomEnabled)
            {
               return mSystem.mRom->Peek(addr); // &mRom.LynxBase;
            }
            else
            {
               return mSystem.mRam->Peek(addr); // &CRam_LynxBase;
        ...
        case 0xFF:
            ...
            {
                return CMemMapPeek(addr); // &CMemMap_LynxBase;
            }
            else if(mVectorsEnabled)
            {
               return mSystem.mRom->Peek(addr); // &mRom.LynxBase;
